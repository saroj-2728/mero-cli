...
export const saveCredentials = async (username: string, password: string, dp: string) => {
  try {
    let credentials = await loadCredentials(); // Load existing credentials
    const newCredential = { username, password, dp };
    ...
    const existingIndex = credentials.findIndex((user: { username: string }) => user.username === username);
...
// Function to check if there are any saved credentials
